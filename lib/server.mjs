...
function detectTool(pkg) {
  ...
  // 1. Global npm
  try {
    const out = execFileSync('npm', ['list', '-g', pkg, '--depth=0'], { stdio: ['pipe', 'pipe', 'ignore'], encoding: 'utf-8' });
  ...
  // 2. npx cache
  try {
    const prefix = execFileSync('npm', ['config', 'get', 'cache'], { stdio: ['pipe', 'pipe', 'ignore'], encoding: 'utf-8' }).trim();
